use crate::pane_manager::MAX_BASE_PANES;
...
// TODO: Folds
// TODO: Marks besides cursor mark per window

pub struct Buffer {
    lines: Vec<String>,
    marks: [Mark; MAX_BASE_PANES],
}
...
impl Mark {
    ...
    pub fn move_row(&mut self, delta: i64) -> &mut Mark {
        Self::move_member(&mut self.row, delta);
        self
    }
    pub fn move_col(&mut self, delta: i64) -> &mut Mark{
        Self::move_member(&mut self.col, delta);
        self
    }
    pub fn clamp_row(&mut self, max: usize) -> &mut Mark {
        if self.row > max {
            self.row = max;
        }
        self
    }
    pub fn clamp_col(&mut self, max: usize) -> &mut Mark{
        if self.col > max {
            self.col = max;
        }
        self
...
impl Buffer {
    ...
    pub fn from_str(s: &str) -> Self {
        Self {
            ...
            marks: std::array::from_fn(|_| Mark::new(0, 0)),
        }
    }
    ...
    pub fn get_mark(&self, key: u8) -> &Mark {
        &self.marks[key as usize]
    }

    pub fn get_mark_mut(&mut self, key: u8) -> &mut Mark {
        &mut self.marks[key as usize]
    }

    pub fn set_mark(&mut self, key: u8, mark: Mark) {
        *self.get_mark_mut(key) = mark;
    }
}
